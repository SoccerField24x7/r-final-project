...
Before digging into answering the actual questions, I plotted the raw data on the world map to provide a visual representation to base my hypothesis on. At a glance, I thought North America would be the continent that had the most meteorite finds. I reasoned that we would have more sightings in NASA data given that it is a North American organization.
...
```
The winner, by a landslide, is Antarctica with over 6 times the number of landings over its nearest competition.
...
The other issue I ran into was trying to convert "South America and the Caribbean" to "South America" - which fits on my axes much better. I initially tried using `ifelse(grepl("^South", continent, ignore.case = TRUE), "South America", continent)` It would have been a perfect solution because it uses control flow AND RegEx, but it would find and do the variable replacement for South America properly, but for all of the other values rather than using the value as expected, it would give me a number. Some type of enumeration that I never could figure out how to get to show the value. I ended up using `str_replace()` instead. 